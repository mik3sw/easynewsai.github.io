...
var getSuggestionUserIdByKey = (key) => isDefined(key) ? key.split(`${BaseSuggestionPlugin.key}_`)[1] : null;
...
var acceptSuggestion = (editor, description) => {
  withoutNormalizing6(editor, () => {
    ...
    unsetNodes2(editor, [BaseSuggestionPlugin.key, suggestionKey], {
      at: [],
      match: (n) => {
        const node = n;
        return node[SUGGESTION_KEYS.id] === description.suggestionId && !node.suggestionDeletion && !!node[suggestionKey];
      }
    });
    removeNodes3(editor, {
      at: [],
...
// src/lib/transforms/addSuggestionMark.ts
import { nanoid as nanoid6 } from "@udecode/plate-common";
var addSuggestionMark = (editor) => {
  var _a, _b, _c;
  if (!editor.selection) return;
  const id = (_a = findSuggestionId(editor, editor.selection)) != null ? _a : nanoid6();
  if (!((_b = editor.marks) == null ? void 0 : _b[BaseSuggestionPlugin.key])) {
    editor.addMark(BaseSuggestionPlugin.key, true);
  }
  if (!((_c = editor.marks) == null ? void 0 : _c[SUGGESTION_KEYS.id])) {
